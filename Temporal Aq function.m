clear; clc;
% =================  Folder  =================
folderName = uigetdir(pwd,'choose folder contains csv files');
if folderName == 0
    return;
end
% =================  parameters =================
...
BendEnergy  = NaN(nFiles,1);   % Bending energy
BRI         = NaN(nFiles,1);   
q_eff       = NaN(nFiles,1);
...
for k = 1:nFiles
    ...
    % ======== bending energy(BRI)========
    BendEnergy(k) = sum(q.^2 .* abs(Aq).^2);

    % ======== BRI========
    BRI(k) = BendEnergy(k);

    % -------- Characteristic q_eff--------
    if BendEnergy(k) > 0
        q_eff(k) = sum(q .* (q.^2 .* abs(Aq).^2)) / BendEnergy(k);
    end
end
...
bar(BRI);
xlabel('Fiber index');
ylabel('Bending energy density (\mum^{-2})');
title('Bending energy per unit length');
grid on;
...
ResultTable = table( ...
    ...
    BRI, ...
    'VariableNames', ...
    {'FileName','FiberLength_um','qc_umInv','q_eff_umInv','BendEnergy','BRI'} );

outFile = fullfile(folderName,'BendingEnergyIndex.xlsx');
